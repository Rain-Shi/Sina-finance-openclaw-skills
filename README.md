...
经实测发现，部分模型（如豆包）倾向于**跳过 SKILL.md 文件读取**，仅根据 description 字段作答。本仓库所有 skill 的 description 字段都内嵌输出结构骨架（如 "① 标题 ② 情绪 ③ 三大指数…"），即使模型不读 SKILL.md，也能按骨架生成。
